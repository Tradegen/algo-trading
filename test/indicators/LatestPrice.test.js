...
describe("LatestPrice", () => {
  ...
  before(async () => {
    ...
    IndicatorFactory = await ethers.getContractFactory('TestLatestPrice');
  ...
  describe("#update", () => {
    ...
    it("meets requirements", async () => {
        ...
        expect(history[0]).to.equal(parseEther("13256.19"));

        let value = await indicator.getValue(1);
        expect(value.length).to.equal(1);
        expect(value[0]).to.equal(parseEther("13256.19"));
    });
    ...
    it("update multiple times", async () => {
        ...
        expect(history[0]).to.equal(parseEther("13256.19"));
        expect(history[1]).to.equal(parseEther("18726.394749"));
        expect(history[2]).to.equal(parseEther("20981"));
        expect(history[3]).to.equal(parseEther("19381"));

        let value = await indicator.getValue(1);
        expect(value.length).to.equal(1);
        expect(value[0]).to.equal(parseEther("19381"));
    });
    ...
    it("update more than MAX_HISTORY_LENGTH times", async () => {
        ...
        expect(history[1]).to.equal(parseEther("30713"));
        expect(history[19]).to.equal(parseEther("32513"));
    });
    ...
    it("multiple instances", async () => {
        ...
        let value = await indicator.getValue(1);
        expect(value[0]).to.equal(parseEther("13748"));
        
        let value2 = await indicator.getValue(2);
        expect(value2[0]).to.equal(parseEther("2984"));
    });
    ...
    it("not ready to update", async () => {
      ...
      let history = await indicator.getHistory(1);
      expect(history.length).to.equal(1);
      expect(history[0]).to.equal(parseEther("14851"));
    });
  });
});*/