...
describe("NthPriceUpdate", () => {
  ...
  before(async () => {
    ...
    IndicatorFactory = await ethers.getContractFactory('TestNthPriceUpdate');
  ...
  describe("#update", () => {
    ...
    it("update more than MAX_HISTORY_LENGTH times", async () => {
        ...
        let value = await indicator.getValue(1);
        expect(value.length).to.equal(1);
        expect(value[0]).to.equal(parseEther("15"));
    });
