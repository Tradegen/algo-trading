...
describe("NPercent", () => {
  ...
  before(async () => {
    ...
    IndicatorFactory = await ethers.getContractFactory('TestNPercent');
  ...
  describe("#createInstance", () => {
    it("onlyComponentRegistry", async () => {
      let tx = indicator.connect(otherUser).createInstance("BTC", 1, 1, [1000]);
      await expect(tx).to.be.reverted;
    });
    ...
    it("param out of bounds", async () => {
        let tx = indicator.createInstance("BTC", 1, 1, [100000]);
        await expect(tx).to.be.reverted;
    });

    it("meets requirements", async () => {
        let tx = await indicator.createInstance("BTC", 1, 1, [1000]);
        await tx.wait();
  ...
  describe("#update", () => {
    it("onlyDedicatedKeeper", async () => {
      ...
      let tx2 = await indicator.createInstance("BTC", 1, 1, [1000]);
      await tx2.wait();
    ...
    it("meets requirements", async () => {
        ...
        let history = await indicator.getHistory(1);
        expect(history.length).to.equal(1);
        expect(history[0]).to.equal(1000);
    ...
    it("update multiple times", async () => {
        ...
        let tx2 = await indicator.createInstance("BTC", 1, 1, [2000]);
        await tx2.wait();
    ...
    it("multiple instances", async () => {
        ...
        let tx3 = await indicator.createInstance("BTC", 1, 1, [200]);
        await tx3.wait();
  
        let tx4 = await indicator.createInstance("ETH", 5, 5, [300]);
        await tx4.wait();
        ...
        let value = await indicator.getValue(1);
        expect(value[0]).to.equal(200);
        
        let value2 = await indicator.getValue(2);
        expect(value2[0]).to.equal(300);
    });
    ...
    it("not ready to update", async () => {
      ...
      let tx2 = await indicator.createInstance("BTC", 1, 1, [3000]);
      await tx2.wait();
      ...
      let history = await indicator.getHistory(1);
      expect(history.length).to.equal(1);
      expect(history[0]).to.equal(3000);
    });
  });
});*/