...
/*
describe("PreviousNPriceUpdates", () => {
  let deployer;
  let otherUser;
  ...
  before(async () => {
    ...
    IndicatorFactory = await ethers.getContractFactory('TestPreviousNPriceUpdates');
  ...
  describe("#createInstance", () => {
    ...
    it("param out of bounds; too low", async () => {
        let tx = indicator.createInstance("BTC", 1, 1, [0]);
        await expect(tx).to.be.reverted;
    });

    it("param out of bounds; too high", async () => {
        let tx = indicator.createInstance("BTC", 1, 1, [25]);
        await expect(tx).to.be.reverted;
    });

    it("meets requirements", async () => {
        let tx = await indicator.createInstance("BTC", 1, 1, [5]);
        await tx.wait();
  ...
  describe("#update", () => {
    ...
    it("meets requirements; not enough history", async () => {
    ...
    it("update multiple times", async () => {
        let tx = await indicator.setKeeper(1, deployer.address);
        await tx.wait();
        ...
        expect(value[1]).to.equal(parseEther("20"));
        expect(value[2]).to.equal(parseEther("30"));
    });
    ...
    it("update multiple times; > param", async () => {
        ...
        let value = await indicator.getValue(1);
        expect(value.length).to.equal(3);
        expect(value[0]).to.equal(parseEther("40"));
        expect(value[1]).to.equal(parseEther("20"));
        expect(value[2]).to.equal(parseEther("30"));
    });
    ...
    it("update more than MAX_HISTORY_LENGTH times", async () => {
        ...
        expect(value[1]).to.equal(parseEther("16"));
        expect(value[19]).to.equal(parseEther("34"));
    });
    ...
    it("multiple instances", async () => {
        ...
        expect(value2.length).to.equal(0);

        let history2 = await indicator.getHistory(2);
        expect(history2.length).to.equal(1);
        expect(history2[0]).to.equal(parseEther("30"));
    });
