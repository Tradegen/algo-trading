...
describe("EMA", () => {
  ...
  before(async () => {
    ...
    IndicatorFactory = await ethers.getContractFactory('TestEMA');
  ...
  describe("#update", () => {
    it("onlyDedicatedKeeper", async () => {
      ...
      let tx2 = await indicator.createInstance("BTC", 1, 1, [19]);
      await tx2.wait();
    ...
    it("update multiple times; latest price > previous EMA", async () => {
        let tx = await indicator.setKeeper(1, deployer.address);
        await tx.wait();
        ...
        expect(history[0]).to.equal(parseEther("10"));
        expect(history[1]).to.equal(parseEther("11"));
        expect(history[2]).to.equal(parseEther("11"));
        expect(history[3]).to.equal(parseEther("11.9"));
    });
    ...
    it("update multiple times; latest price < previous EMA", async () => {
        ...
        let tx6 = await candlestickDataFeedRegistry.setPrice("BTC", 1, parseEther("5"));
        await tx6.wait();
        ...
        let tx9 = await candlestickDataFeedRegistry.setPrice("BTC", 1, parseEther("4.5"));
        await tx9.wait();
        ...
        let tx12 = await candlestickDataFeedRegistry.setPrice("BTC", 1, parseEther("4.5"));
        await tx12.wait();
        ...
        expect(history.length).to.equal(4);
        expect(history[0]).to.equal(parseEther("10"));
        expect(history[1]).to.equal(parseEther("9.5"));
        expect(history[2]).to.equal(parseEther("9.45"));
        expect(history[3]).to.equal(parseEther("9"));
    });

    it("update multiple times; latest price < previous EMA, then latest price > previous EMA", async () => {
        let tx = await indicator.setKeeper(1, deployer.address);
        await tx.wait();
        ...
        let tx9 = await candlestickDataFeedRegistry.setPrice("BTC", 1, parseEther("14.5"));
        await tx9.wait();
        ...
        let tx12 = await candlestickDataFeedRegistry.setPrice("BTC", 1, parseEther("24.5"));
        await tx12.wait();
        ...
        expect(history[0]).to.equal(parseEther("10"));
        expect(history[1]).to.equal(parseEther("9.5"));
        expect(history[2]).to.equal(parseEther("10.45"));
        expect(history[3]).to.equal(parseEther("11"));
    });
    ...
    it("update more than MAX_HISTORY_LENGTH times", async () => {
        ...
        expect(history[1]).to.equal(parseEther("10.39"));
        expect(history[19]).to.equal(parseEther("18.96240132571"));
    });
    ...
    it("multiple instances", async () => {
        ...
        let tx3 = await indicator.createInstance("BTC", 1, 1, [19]);
        await tx3.wait();
  
        let tx4 = await indicator.createInstance("ETH", 5, 5, [19]);
        await tx4.wait();
