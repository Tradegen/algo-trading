...
describe("Closes", () => {
  ...
  let PeviousNPriceUpdatesFactory;
  ...
  before(async () => {
    ...
    DownFactory = await ethers.getContractFactory('Down');
    ...
    ComparatorFactory = await ethers.getContractFactory('TestCloses');
  ...
  beforeEach(async () => {
    down = await DownFactory.deploy(deployer.address, candlestickDataFeedRegistryAddress, deployer.address);
    await down.deployed();
    downAddress = down.address;

    up = await UpFactory.deploy(deployer.address, candlestickDataFeedRegistryAddress, deployer.address);
    await up.deployed();
  ...
  describe("#createInstance", () => {
    it("onlyComponentRegistry", async () => {
      let tx = comparator.connect(otherUser).createInstance(latestPriceAddress, downAddress, 1, 1);
      await expect(tx).to.be.reverted;
    });

    it("indicators are the same", async () => {
        let tx = await down.createInstance("BTC", 1, 1, []);
        await tx.wait();

        let tx2 = comparator.createInstance(downAddress, downAddress, 1, 1);
    ...
    it("invalid timeframe", async () => {
        ...
        let tx3 = comparator.createInstance(latestPriceAddress, downAddress, 2, 2);
    ...
    it("meets requirements; same timeframe", async () => {
        ...
        let tx3 = await comparator.createInstance(latestPriceAddress, downAddress, 1, 1);
        await tx3.wait();
        ...
        expect(state[1]).to.equal(downAddress);
    ...
    it("meets requirements; different timeframe", async () => {
        let tx = await down.createInstance("BTC", 10, 10, []);
        await tx.wait();
  ...
  describe("#checkConditions", () => {
    it("onlyDedicatedKeeper", async () => {
      ...
      let tx2 = await down.createInstance("BTC", 10, 10, []);
      await tx2.wait();

        let tx3 = await latestPrice.createInstance("BTC", 5, 5, []);
        await tx3.wait();

        let tx4 = await comparator.createInstance(latestPriceAddress, downAddress, 1, 1);
        await tx4.wait();
    ...
    it("Up; one price history; meets conditions", async () => {
        let tx = await up.setKeeper(1, deployer.address);
        await tx.wait();

        let tx2 = await up.createInstance("BTC", 1, 1, []);
        await tx2.wait();
        ...
        let tx6 = await comparator.createInstance(latestPriceAddress, upAddress, 1, 1);
        await tx6.wait();

        let tx7 = await up.update(1);
        await tx7.wait();
        ...
        let state = await comparator.getState(1);
        expect(state[4][0]).to.equal(parseEther("10"));
    });

    it("Up; one price history; multi update; fails conditions", async () => {
        let tx = await up.setKeeper(1, deployer.address);
        await tx.wait();
        ...
        let tx11 = await comparator.setLastUpdated(1, 0);
        await tx11.wait();

        let tx12 = await candlestickDataFeedRegistry.setPrice("BTC", 1, parseEther("1"));
        await tx12.wait();

        let tx13 = await latestPrice.setLastUpdated(1, 0);
        await tx13.wait();

        let tx14 = await latestPrice.update(1);
        await tx14.wait();
        ...
        let state = await comparator.getState(1);
        expect(state[4][0]).to.equal(parseEther("1"));
    });

    it("Up; one price history; multi update; meets conditions", async () => {
        let tx = await up.setKeeper(1, deployer.address);
        await tx.wait();
        ...
        let tx12 = await candlestickDataFeedRegistry.setPrice("BTC", 1, parseEther("20"));
        await tx12.wait();
        ...
        let state = await comparator.getState(1);
        expect(state[4][0]).to.equal(parseEther("20"));
    });

    it("Down; one price history; multi update; fails conditions", async () => {
        let tx = await down.setKeeper(1, deployer.address);
        await tx.wait();

        let tx2 = await down.createInstance("BTC", 1, 1, []);
        await tx2.wait();
        ...
        let tx6 = await comparator.createInstance(latestPriceAddress, downAddress, 1, 1);
        await tx6.wait();

        let tx7 = await down.update(1);
        await tx7.wait();
    ...
    it("Down; one price history; multi update; meets conditions", async () => {
        let tx = await down.setKeeper(1, deployer.address);
        await tx.wait();
        ...
        let tx12 = await candlestickDataFeedRegistry.setPrice("BTC", 1, parseEther("2"));
        await tx12.wait();
        ...
        let state = await comparator.getState(1);
        expect(state[4][0]).to.equal(parseEther("2"));
    });

    it("Up; multi price history; meets conditions", async () => {
        let tx = await up.setKeeper(1, deployer.address);
        await tx.wait();
        ...
        let tx6 = await comparator.createInstance(previousNPriceUpdatesAddress, upAddress, 1, 1);
        await tx6.wait();
        ...
        let tx9 = await candlestickDataFeedRegistry.setPrice("BTC", 1, parseEther("15"));
        await tx9.wait();

        let tx10 = await previousNPriceUpdates.setLastUpdated(1, 0);
        await tx10.wait();
        ...
        let tx13 = await previousNPriceUpdates.setLastUpdated(1, 0);
        await tx13.wait();
    ...
    it("Up; multi price history; fails conditions", async () => {
        let tx = await up.setKeeper(1, deployer.address);
        await tx.wait();
        ...
        let tx9 = await candlestickDataFeedRegistry.setPrice("BTC", 1, parseEther("5"));
        await tx9.wait();
    ...
    it("Down; multi price history; meets conditions", async () => {
        let tx = await down.setKeeper(1, deployer.address);
        await tx.wait();
        ...
        let tx6 = await comparator.createInstance(previousNPriceUpdatesAddress, downAddress, 1, 1);
        await tx6.wait();
    ...
    it("Down; multi price history; fails conditions", async () => {
        let tx = await down.setKeeper(1, deployer.address);
        await tx.wait();
    ...
    it("indicator not supported", async () => {
        let tx = await interval.setKeeper(1, deployer.address);
        await tx.wait();

        let tx2 = await interval.createInstance("BTC", 1, 1, [parseEther("10000"), 1]);
        await tx2.wait();
