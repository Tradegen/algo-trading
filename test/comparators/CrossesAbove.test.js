...
describe("CrossesAbove", () => {
  ...
  before(async () => {
    ...
    ComparatorFactory = await ethers.getContractFactory('TestCrossesAbove');
  ...
  describe("#checkConditions", () => {
    ...
    it("One update; meets conditions", async () => {
        ...
        let state = await comparator.getState(1);
        expect(state[4][0]).to.equal(parseEther("1200"));
    ...
    it("Multi update; meets conditions", async () => {
        ...
        let tx16 = await candlestickDataFeedRegistry.setPrice("BTC", 1, parseEther("1400"));
        await tx16.wait();

        let tx17 = await latestPrice.setLastUpdated(1, 0);
        await tx17.wait();

        let tx18 = await latestPrice.update(1);
        await tx18.wait();

        let tx19 = await comparator.setLastUpdated(1, 0);
        await tx19.wait();

        let tx20 = await comparator.checkConditions(1);
        await tx20.wait();
