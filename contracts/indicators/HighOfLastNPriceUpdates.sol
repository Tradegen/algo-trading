...
contract HighOfLastNPriceUpdates is IIndicator {
    ...
    */
    function update(uint256 _instance) external override onlyDedicatedKeeper(_instance) returns (bool) {
        ...
        (, uint256 latestHighPrice,,,,,) = candlestickDataFeedRegistry.getCurrentCandlestick(data.asset, data.assetTimeframe);

        // Return early if there was an error getting the latest price.
        if (latestHighPrice == 0) {
            return false;
        }

        instances[_instance].history.push(latestHighPrice);
        ...
        for (uint256 i = 0; i < length; i++) {
            high = (instances[_instance].history[historyLength - i - 1] > high) ? instances[_instance].history[historyLength - i - 1] : high;
        }

        instances[_instance].value = high;
        lastUpdated[_instance] = block.timestamp;
        
        emit Updated(_instance, latestHighPrice, high);
