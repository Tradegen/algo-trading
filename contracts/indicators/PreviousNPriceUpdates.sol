...
contract PreviousNPriceUpdates is IIndicator {
    ...
    */
    function getValue(uint256 _instance) external view override returns (uint256[] memory) {
        ...
        uint256[] memory result = new uint256[]((historyLength >= instances[_instance].params[0]) ? instances[_instance].params[0] : 0);
        
        for (uint256 i = 0; i < result.length; i++) {
            result[i] = history[_instance][historyLength.sub(N).add(i)];
        }
    ...
    */
    function update(uint256 _instance) external override onlyDedicatedKeeper(_instance) returns (bool) {
        ...
        history[_instance][updateCount[_instance]] = latestPrice;
