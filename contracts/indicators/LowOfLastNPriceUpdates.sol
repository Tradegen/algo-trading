...
contract LowOfLastNPriceUpdates is IIndicator {
    ...
    */
    function update(uint256 _instance) external override onlyDedicatedKeeper(_instance) returns (bool) {
        ...
        (,, uint256 latestLowPrice,,,,) = candlestickDataFeedRegistry.getCurrentCandlestick(data.asset, data.assetTimeframe);
        ...
        instances[_instance].history.push(latestLowPrice);
        ...
        for (uint256 i = 0; i < length; i++) {
            low = (instances[_instance].history[historyLength - i - 1] < low) ? instances[_instance].history[historyLength - i - 1] : low;
        }

        instances[_instance].value = low;
        lastUpdated[_instance] = block.timestamp;
        
        emit Updated(_instance, latestLowPrice, low);
